...
type TunnelCard = THREE.Mesh<THREE.PlaneGeometry, THREE.MeshBasicMaterial> & {
...
export class InfiniteTunnel {
  ...
  private scrollMinHeight = 0;
  private currentCameraZ = 0;
  private previews: QueuePreview[] = [];
  private previewFingerprint = '';
  private previewById = new Map<string, QueuePreview>();
  private refreshTimer: number | null = null;
  private visibilityHandler: (() => void) | null = null;
  private raycaster = new THREE.Raycaster();
  private pointer = new THREE.Vector2();
  private hoverColor = new THREE.Color(0xffffff);
  private scratchColor = new THREE.Color();
  private container: HTMLElement;
  private animationId = 0;
  private isDarkMode = false;
  private onCardClick: ((queueId: string) => void) | null = null;
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (Array.isArray(next)) {
          ...
          const ids = previews.map((item: any) => String(item?.id ?? '')).filter(Boolean);
  ...
  private dispatchPreviewStatus(previews: QueuePreview[], hasLivePreviews: boolean): void {
  ...
  private shouldPlaceCard(segmentIndex: number, slotId: number, threshold: number): boolean {
  ...
  private startRefreshLoop(): void {
    ...
    this.refreshTimer = window.setInterval(async () => {
      if (!this.isLandingActive()) return;
  ...
  private createSegment(index: number): THREE.Group {
  ...
  private populateCards(segment: THREE.Group, segmentIndex: number, resetAssignments = false): void {
    // Cleanup old cards
    const toRemove: THREE.Object3D[] = [];
    segment.traverse((child: THREE.Object3D) => { if (child.name === 'card') toRemove.push(child); });
    ...
    // Helper to add card
    const addCard = (pos: THREE.Vector3, rot: THREE.Euler, width: number, height: number, seedIdx: number, slotId: number) => {
    ...
    for (let i = 0; i < FLOOR_COLS; i++) {
        ...
        if (this.shouldPlaceCard(segmentIndex, i, 0.8)) { 
            ...
            addCard(
               new THREE.Vector3(x, -h + 0.05, -d/2), // Slightly raised
    ...
    for (let i = 0; i < WALL_ROWS; i++) {
        ...
        if (this.shouldPlaceCard(segmentIndex, slotId, 0.8)) {
            ...
            addCard(
                new THREE.Vector3(-w + 0.05, y, -d/2),
    ...
    for (let i = 0; i < FLOOR_COLS; i++) {
        ...
        if (this.shouldPlaceCard(segmentIndex, slotId, 0.9)) {
            const x = -w + i * COL_WIDTH + COL_WIDTH / 2;
            addCard(
                new THREE.Vector3(x, h - 0.05, -d/2),
                new THREE.Euler(Math.PI / 2, 0, 0),
                COL_WIDTH - cellMargin,
                d - cellMargin,
                slotId,
  ...
  private createCard(preview: QueuePreview, width: number, height: number, pulseSeed: number): TunnelCard {
    ...
    const card = new THREE.Mesh(geometry, material) as TunnelCard;
    card.name = 'card';
    card.userData = {
      queueId: preview.id,
      title: preview.title,
  ...
  private handleClick = (): void => {
    ...
    this.segments.forEach(seg => {
      seg.traverse((child: THREE.Object3D) => {
        if (child.name === 'card') cards.push(child);
      });
    });
  ...
  private animate = (): void => {
    ...
    this.segments.forEach(seg => {
      seg.traverse((child: THREE.Object3D) => {
        if (child.name === 'card') {
          cards.push(child);
          const card = child as TunnelCard;
    ...
    const intersects = this.raycaster.intersectObjects(cards);
    if (intersects.length > 0) {
      const hovered = intersects[0].object as TunnelCard;
  ...
  setTheme(dark: boolean): void {
    ...
    this.segments.forEach(seg => {
        const grid = seg.getObjectByName('grid') as THREE.LineSegments;
        if (grid) {
  ...
  private ensureScrollRange(): void {
    if (!this.isLandingActive()) return;
  ...
  private extendScrollRangeIfNeeded(): void {
    if (!this.isLandingActive()) return;
    if (!this.scrollRange) this.ensureScrollRange();
  ...
  private isLandingActive(): boolean {
