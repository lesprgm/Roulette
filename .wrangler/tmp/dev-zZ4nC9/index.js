...
var KVDedupe = class {
  static {
    __name(this, "KVDedupe");
  }
  kv;
  maxItems;
  constructor(kv, maxItems = 200) {
    this.kv = kv;
...
// src/utils.ts
function extractCompletedObjects(text) {
  const objs = [];
  let s = text.trim();
  if (s.startsWith("[")) {
...
__name(extractCompletedObjects, "extractCompletedObjects");
async function getDesignMatrixB64(env) {
  if (env.ASSETS_KV) {
    return await env.ASSETS_KV.get("design_matrix_b64");
  }
  return null;
}
__name(getDesignMatrixB64, "getDesignMatrixB64");
var RateLimiter = class {
  static {
    __name(this, "RateLimiter");
  }
  static async check(env, key) {
    if (env.LIMITER_KV) {
      const now = Date.now();
      const bucketStr = await env.LIMITER_KV.get(`limit:${key}`);
      const bucket = bucketStr ? JSON.parse(bucketStr) : null;
      if (!bucket || bucket.reset < now) {
        const newBucket = { count: 1, reset: now + 6e4 };
        await env.LIMITER_KV.put(`limit:${key}`, JSON.stringify(newBucket));
        return { allowed: true, remaining: 9, reset: newBucket.reset / 1e3 };
      }
      if (bucket.count >= 10) {
        return { allowed: false, remaining: 0, reset: bucket.reset / 1e3 };
      }
      bucket.count += 1;
      await env.LIMITER_KV.put(`limit:${key}`, JSON.stringify(bucket));
      return { allowed: true, remaining: 10 - bucket.count, reset: bucket.reset / 1e3 };
    }
    return { allowed: true, remaining: 9999, reset: (Date.now() + 6e4) / 1e3 };
  }
};
...
async function getNextCategory(env) {
...
__name(getNextCategory, "getNextCategory");
var PAGE_SHAPE_HINT = `
...
`;
async function* generatePageBurst(env, seed, brief = "") {
...
${PAGE_SHAPE_HINT}`;
  const userPrompt = brief ? `Create a ${category} experience about: ${brief}. Seed: ${seed}` : `Create a unique, creative ${category} experience. Seed: ${seed}`;
...
__name(generatePageBurst, "generatePageBurst");
async function* callGeminiBurst(env, system, user, seed, imageB64) {
...
__name(callGeminiBurst, "callGeminiBurst");
function validateAndStandardize(obj, seed) {
...
__name(validateAndStandardize, "validateAndStandardize");
async function callGroq(env, system, user, seed) {
  const models = [
...
async function callOpenRouter(env, system, user, seed) {
  const models = [
...
__name(parseGeneratedContent, "parseGeneratedContent");
async function maybeRunComplianceReview(env, page, brief, category) {
...
__name(maybeRunComplianceReview, "maybeRunComplianceReview");
async function runComplianceBatch(env, pages, brief = "") {
...
__name(runComplianceBatch, "runComplianceBatch");
...
var PrefetchQueue = class {
  ...
  async dequeue() {
    ...
    const [storageKey, id] = Array.from(list.entries())[0];
...
app.post("/generate", async (c) => {
  try {
    ...
    if (!limit.allowed) {
      c.header("Retry-After", String(Math.floor(limit.reset - Date.now() / 1e3)));
    ...
    if (queueResp.ok) {
      const page2 = await queueResp.json();
      await incrementCounter(c.env.ASSETS_KV);
      return c.json(page2);
    }
    const iterator = generatePageBurst(c.env, seed, brief);
...
// .wrangler/tmp/bundle-N6ToHr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-N6ToHr/middleware-loader.entry.ts
