...
- **Provider order & fallbacks**
  ...
  2. **Gemini Bursting**: Technically the primary model for **Burst Streaming**. Gemini is the only provider that attempts to batch multiple sites per request (currently up to 10). It is skipped for ad-hoc requests unless fallbacks fail.
...
- **Batching**: A single Gemini request generates up to **10** items in a JSON array.
...
- **Review Batching**: Follow-up sites are reviewed in chunks of `PREFETCH_REVIEW_BATCH` (default 3). This keeps individual review calls small and predictable even when a burst produces many pages.
