...
def score_activity_depth(doc: Dict[str, Any], plan: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    variant_patterns = {
        ...
        "sudoku_grid": r"\b(sudoku|number|grid|row|column|box|puzzle)\b",
        "connect_four": r"\b(connect|four|disc|drop|column|win|row)\b",
        "solitaire_card": r"\b(solitaire|card|stack|suit|draw|tableau)\b",
        "pong_clone": r"\b(pong|paddle|ball|bounce|score|point)\b",
        "flappy_bird": r"\b(flappy|bird|flap|pipe|dodge|fall)\b",
        ...
        "task_board": r"\b(task|board|column|card|status|move)\b",
        "sales_tracker": r"\b(sales|lead|deal|pipeline|customer|stage)\b",
        "invoice_builder": r"\b(invoice|line item|subtotal|tax|total|client)\b",
        "dashboard_app": r"\b(dashboard|chart|metric|filter|data|report)\b",
        ...
        "hiring_tracker": r"\b(hiring|candidate|resume|screen|shortlist)\b",
        "helpdesk_app": r"\b(helpdesk|ticket|support|priority|assign|queue)\b",
